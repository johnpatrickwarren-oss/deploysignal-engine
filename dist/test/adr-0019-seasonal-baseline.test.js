...
const seasonal_baseline_1 = require("../baseline/seasonal-baseline");
function lcg(seed) {
    let s = seed >>> 0;
...
const variance = (a) => { const m = a.reduce((x, y) => x + y, 0) / a.length; return a.reduce((s, x) => s + (x - m) ** 2, 0) / a.length; };
const NB = 24, DAYS = 42; // 24 bins (hour-of-day), 6 weeks
function diurnalSeries(seed) {
...
(0, node_test_1.test)('seasonal baseline: removes the diurnal structure (residual variance ≪ raw)', () => {
    ...
    const resid = (0, seasonal_baseline_1.seasonalBaselineResidual)(values, context, bl);
    const ratio = variance(resid) / variance(values);
    strict_1.default.ok(ratio < 0.1, `residual/raw variance should be ≪ 1 (seasonality removed); got ${ratio.toFixed(3)}`);
    // every bin is strict (42 samples each) and the per-bin clean mean tracks the seasonal shape
    strict_1.default.ok(bl.bins.every((b) => b.confidence === 'strict'), 'all bins should be strict at 42 samples/bin');
});
(0, node_test_1.test)('seasonal baseline: clean-null trims anomalies', () => {
    const { values, context } = diurnalSeries(2);
    // corrupt bin 5 with a few large anomalies
    const v = values.slice();
    let injected = 0;
    for (let i = 0; i < v.length && injected < 5; i++)
        if (context[i] === 5) {
            v[i] += 100;
            injected++;
        }
    const clean = (0, seasonal_baseline_1.compileSeasonalBaseline)(values, context, { nBins: NB, minStrict: 30, minPooled: 10 });
    const dirty = (0, seasonal_baseline_1.compileSeasonalBaseline)(v, context, { nBins: NB, minStrict: 30, minPooled: 10 });
    // the trimmed bin-5 mean stays close to the uncorrupted mean (anomalies dropped), and clean n < 42
    strict_1.default.ok(Math.abs(dirty.bins[5].mean - clean.bins[5].mean) < 2, `clean-null should reject the anomalies; mean drifted ${Math.abs(dirty.bins[5].mean - clean.bins[5].mean).toFixed(2)}`);
    strict_1.default.ok(dirty.bins[5].n <= 42 - injected, `clean count should drop the ${injected} anomalies; got n=${dirty.bins[5].n}`);
});
(0, node_test_1.test)('seasonal baseline: sparse bins fall back to aggregate', () => {
    // only bins 0..3 populated heavily; bins 4..23 get 1 sample each ⇒ aggregate
    const values = [], context = [];
    for (let i = 0; i < 200; i++) {
        values.push(50 + gaussian(lcg(i + 1)));
        context.push(i % 4);
    }
    for (let h = 4; h < NB; h++) {
        values.push(99);
    ...
    const bl = (0, seasonal_baseline_1.compileSeasonalBaseline)(values, context, { nBins: NB, minStrict: 30, minPooled: 10 });
    strict_1.default.equal(bl.bins[0].confidence, 'strict');
...
(0, node_test_1.test)('seasonal baseline: a fault in fresh data survives residualisation', () => {
    ...
    const fresh = [], fctx = [];
    const rng = lcg(77), g = () => gaussian(rng);
    for (let h = 0; h < NB; h++) {
        const seasonal = 10 * Math.sin((2 * Math.PI * h) / NB) + 3 * Math.cos((4 * Math.PI * h) / NB);
        fresh.push(50 + seasonal + g() + 8);
        fctx.push(h);
    }
    const resid = (0, seasonal_baseline_1.seasonalBaselineResidual)(fresh, fctx, bl);
    const m = resid.reduce((a, b) => a + b, 0) / resid.length;
    strict_1.default.ok(m > 6, `the +8 fault should survive (seasonality removed, fault kept); residual mean ${m.toFixed(2)}`);
});
(0, node_test_1.test)('seasonal baseline: guards', () => {
    const { values, context } = diurnalSeries(1);
    strict_1.default.throws(() => (0, seasonal_baseline_1.compileSeasonalBaseline)([], [], { nBins: NB }), /non-empty/);
    strict_1.default.throws(() => (0, seasonal_baseline_1.compileSeasonalBaseline)(values, context.slice(1), { nBins: NB }), /context length/);
    strict_1.default.throws(() => (0, seasonal_baseline_1.compileSeasonalBaseline)(values, context, { nBins: 0 }), /nBins/);
    strict_1.default.throws(() => (0, seasonal_baseline_1.compileSeasonalBaseline)(values, context.map(() => 99), { nBins: NB }), /out of range/);
    strict_1.default.throws(() => (0, seasonal_baseline_1.compileSeasonalBaseline)(values, context, { nBins: NB, minStrict: 5, minPooled: 10 }), /minPooled <= minStrict/);
});
//# sourceMappingURL=adr-0019-seasonal-baseline.test.js.map