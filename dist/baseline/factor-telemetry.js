...
exports.validateFactorTelemetry = validateFactorTelemetry;
exports.resolveFactorMembership = resolveFactorMembership;
exports.alignToGrid = alignToGrid;
...
function validateFactorTelemetry(ft, ticks) {
    const fn = 'validateFactorTelemetry';
    if (!Number.isInteger(ticks) || ticks < 1)
        throw new RangeError(`${fn}: ticks must be a positive integer; got ${ticks}`);
    if (ft.ticks !== ticks)
        throw new RangeError(`${fn}: telemetry ticks ${ft.ticks} != analysis grid ticks ${ticks}`);
    if (!(ft.dt > 0))
        throw new RangeError(`${fn}: dt must be > 0; got ${ft.dt}`);
    if (!Number.isFinite(ft.t0))
        throw new RangeError(`${fn}: t0 must be finite`);
    ...
    for (const id of ft.factorIds) {
        if (seen.has(id))
            throw new RangeError(`${fn}: duplicate factor id "${id}"`);
        seen.add(id);
    }
    for (let k = 0; k < ft.signals.length; k++) {
        if (ft.signals[k].length !== ticks)
            throw new RangeError(`${fn}: factor "${ft.factorIds[k]}" has length ${ft.signals[k].length}, expected ${ticks}`);
        for (let t = 0; t < ticks; t++)
            if (!Number.isFinite(ft.signals[k][t]))
                throw new RangeError(`${fn}: non-finite value for factor "${ft.factorIds[k]}" at tick ${t}`);
    }
}
...
function resolveFactorMembership(factorIds, membershipByFactorId) {
    const index = new Map();
    factorIds.forEach((id, i) => index.set(id, i));
    return membershipByFactorId.map((ids, shard) => ids.map((id) => {
        const k = index.get(id);
        if (k === undefined)
            throw new RangeError(`resolveFactorMembership: shard ${shard} references unknown factor id "${id}"`);
        return k;
    }));
}
...
function alignToGrid(samples, t0, dt, ticks, opts) {
    const fn = 'alignToGrid';
    if (!(dt > 0))
        throw new RangeError(`${fn}: dt must be > 0; got ${dt}`);
    if (!Number.isInteger(ticks) || ticks < 1)
        throw new RangeError(`${fn}: ticks must be a positive integer; got ${ticks}`);
    ...
    for (const x of s)
        if (!Number.isFinite(x.t) || !Number.isFinite(x.v))
            throw new RangeError(`${fn}: non-finite sample`);
    const out = new Array(ticks);
    let j = 0; // index of the last sample at or before the current grid time
    for (let i = 0; i < ticks; i++) {
        const gt = t0 + i * dt;
        while (j + 1 < s.length && s[j + 1].t <= gt)
            j++;
        const prev = (j < s.length && s[j].t <= gt) ? s[j] : null;
        const next = (() => { let k = prev ? j + 1 : 0; while (k < s.length && s[k].t < gt)
            k++; return k < s.length ? s[k] : null; })();
        if (method === 'linear' && prev && next && next.t > prev.t) {
            if (gt - prev.t > maxGap && next.t - gt > maxGap) {
                out[i] = NaN;
                continue;
            }
...
//# sourceMappingURL=factor-telemetry.js.map