...
exports.TUKEY_C = void 0;
exports.robustLocation = robustLocation;
exports.perShardLevel = perShardLevel;
exports.contaminationRobustResiduals = contaminationRobustResiduals;
...
function robustLocation(xs, c = exports.TUKEY_C) {
    if (!(c > 0))
        throw new RangeError(`robustLocation: c must be > 0; got ${c}`);
    if (xs.length === 0)
        return 0;
    let mu = median(xs);
    ...
    for (let it = 0; it < IRLS_MAX_ITER; it++) {
        ...
        if (wsum === 0)
            break; // all points rejected (pathological) — keep the median start
...
function contaminationRobustResiduals(X, calLen) {
    const n = X.length;
    if (n === 0)
        throw new RangeError('contaminationRobustResiduals: X must have at least one shard');
    const t = X[0].length;
    if (t === 0)
        throw new RangeError('contaminationRobustResiduals: shard rows must be non-empty');
    for (let i = 0; i < n; i++) {
        if (X[i].length !== t) {
    ...
    const colAdj = new Array(n);
    for (let j = 0; j < t; j++) {
        for (let i = 0; i < n; i++)
            colAdj[i] = X[i][j] - lvl[i];
...
//# sourceMappingURL=common-mode.js.map