...
(0, node_test_1.test)('M4: repeat-firing single shard in the pre-window does not suppress a multi-shard post elevation', () => {
    ...
    strict_1.default.equal(result.candidates.length, 1, 'candidate suppressed by event-vs-shard unit mismatch');
    const c = result.candidates[0];
    strict_1.default.equal(c.pre_window_count, 1, 'pre_window_count must count distinct shards');
    strict_1.default.equal(c.post_window_count, 2);
    strict_1.default.deepEqual(c.member_shard_ids, ['gpu:n2:0', 'gpu:n3:0']);
});
(0, node_test_1.test)('M4: pre-window distinct-shard counting keeps boundary semantics (fire at event_ts is post)', () => {
    const result = (0, event_conditional_attribution_1.attributeEventConditional)({
    ...
    strict_1.default.equal(result.candidates[0].pre_window_count, 0);
    strict_1.default.equal(result.candidates[0].post_window_count, 2);
});
//# sourceMappingURL=event-conditional-attribution.test.js.map