...
// ─── Tessera tools/per-shard-whitening.ts: the estimator and the whitener (verbatim) ───────────────
...
export function estimateContrastAr1(samples: ReadonlyArray<number>): ContrastAr1Fit {
 ...
 *  unchanged. Pure. */
export function whitenContrast(x: number, xPrev: number | null, phi: number): number {
  return xPrev === null ? x : x - phi * xPrev;
}

// ─── Tessera tools/contrast.ts (verbatim) ──────────────────────────────────────────────────────────

export const median = (xs: ReadonlyArray<number>): number => { const s = xs.slice().sort((a, b) => a - b); return s[s.length >> 1]; };
export const madScale = (xs: ReadonlyArray<number>): number => { const m = median(xs); return Math.max(1.4826 * median(xs.map((x) => Math.abs(x - m))), 1e-9); };
...
export function fitContrast(d0: ReadonlyArray<number>): ContrastFit {
...
export function applyContrast(d: ReadonlyArray<number>, fit: ContrastFit): number[] {
...
const mean = (xs: ReadonlyArray<number>): number => xs.reduce((a, b) => a + b, 0) / xs.length;
const sd = (xs: ReadonlyArray<number>): number => { const m = mean(xs); return Math.sqrt(xs.reduce((a, b) => a + (b - m) ** 2, 0) / xs.length); };
 ...
 *  healthy baseline. */
export function fitContrastFast(d0: ReadonlyArray<number>): ContrastFit {
...
// ─── Engine additions: the pair, the residual, the envelope ───────────────────────────────────────
...
export interface ContrastPair { treatment: ReadonlyArray<number>; control: ReadonlyArray<number> }
...
export function contrastOf(p: ContrastPair): number[] {
...
export function contrastResidual(p: ContrastPair, fit: ContrastFit): number[] {
...
export interface ContrastAdmission {
  ...
  construction: 'family_A_mixture_supermartingale' | 'family_A_betting_e_process' | 'e_sr_mean_shift'
    | 'calibration_monitor_gaussian' | 'calibration_monitor_bounded';
...
export const CONTRAST_NULL_RUN = 'run-20260905T061348Z';
...
export const CONTRAST_NULL_ENVELOPE: Readonly<ContrastNullEnvelope> = Object.freeze({
  baseline: 'plug-in',
  ...
  evidence: `validation/contrast-null ${CONTRAST_NULL_RUN} (registered 2416bef; P1 FAILED 8 of 21 cells, P3 FAILED 14 of 21 — the estimated offset, not the shared component; nothing admitted under §6)`,
