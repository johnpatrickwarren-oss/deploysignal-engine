...
export const F_RATIO_THRESHOLD = 2.0;     // innovation-variance ratio (test/cal)
export const TREND_T_THRESHOLD = 4.0;     // |OLS slope| / slope-se on whitened test innovations
export const COLLAPSE_SIGMA_THRESHOLD = 6.0; // downward drop of the test mean below cal, in cal-σ units
...
function variance(xs: ReadonlyArray<number>, mu: number): number {
...
export function distributionalSignature(
  ...
  if (cal.len < 3) throw new RangeError(`distributionalSignature: cal.len must be >= 3; got ${cal.len}`);
  if (test.len < 2) throw new RangeError(`distributionalSignature: test.len must be >= 2; got ${test.len}`);
  if (test.start < 1) {
  ...
  if (cal.start < 0 || cal.start + cal.len > values.length || test.start + test.len > values.length) {
    throw new RangeError(
      `distributionalSignature: window out of bounds (values.length=${values.length}, `
      + `cal=[${cal.start},${cal.start + cal.len}), test=[${test.start},${test.start + test.len}))`,
    );
  }
  for (let t = cal.start; t < cal.start + cal.len; t++) {
    if (!Number.isFinite(values[t])) throw new RangeError(`distributionalSignature: non-finite value at calibration index ${t}`);
  }
  for (let t = test.start - 1; t < test.start + test.len; t++) {
    if (!Number.isFinite(values[t])) throw new RangeError(`distributionalSignature: non-finite value at test index ${t}`);
  }

  const calValues = values.slice(cal.start, cal.start + cal.len);
  const phi = computePerSignalAr1Phi(calValues, mean(calValues));
  ...
  let stt = 0, sty = 0;
  for (let k = 0; k < nt; k++) { stt += (k - kbar) ** 2; sty += (k - kbar) * (wt[k] - wtbar); }
  ...
  return {
    ...
    trendT,
    collapseSigma,
    hasSignature: fRatio > F_RATIO_THRESHOLD || trendT > TREND_T_THRESHOLD || collapseSigma > COLLAPSE_SIGMA_THRESHOLD,
  };
}
