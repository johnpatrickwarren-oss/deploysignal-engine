...
import { mixtureConfidenceSequenceAt } from '../detectors/mixture-confidence-sequence';
...
export function eByLevel(delta: number, selectedCount: number, K: number): number {
  if (!(Number.isInteger(K) && K >= 1)) throw new Error(`eByLevel: K must be an integer ≥ 1, got ${K}`);
  ...
  if (!(delta > 0 && delta < 1)) throw new Error(`eByLevel: delta must be in (0,1), got ${delta}`);
...
export function eBenjaminiYekutieli(
  selected: ReadonlyArray<EBySelected>, K: number, delta: number,
): EByOutput {
  const alpha_i = eByLevel(delta, selected.length, K);
  const intervals: EByInterval[] = selected.length === 0 ? [] : selected.map((s) => {
    const cs = mixtureConfidenceSequenceAt(s.level_free, alpha_i);
