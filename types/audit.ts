...
// ADR 0033, v0.8.0-pre: the DeploySignal instance of the registry (its DETECTOR_REGISTRY constant,
// the DetectorId union, the LEGACY_DEPLOYSIGNAL_* lists) no longer lives here. DeploySignal owns its
// registry in its own tree (engine/types/audit.ts there), and no other consumer imported the
// instance. A consumer builds its registry with `detectorRegistryFor({ signals })`; detector ids in
// records are strings whose guarantee row is `guaranteeFor(id)`.
