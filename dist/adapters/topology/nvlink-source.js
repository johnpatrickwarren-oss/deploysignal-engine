...
exports.NvlinkTopologySource = void 0;
exports.parseNvlinkStatus = parseNvlinkStatus;
exports.ingestNvlinkErrorCounter = ingestNvlinkErrorCounter;
...
function parseNvlinkStatus(rawText, opts = {}) {
    const nodes = [];
    ...
    for (const [a, b] of rawEdgePairs) {
        if (a === b)
            continue; // ignore self-peer (shouldn't occur but defend)
        const from = a < b ? a : b;
        const to = a < b ? b : a;
        const key = `${from}|${to}`;
        if (edgeKeys.has(key))
...
class NvlinkTopologySource {
    constructor(rawText, opts = {}) {
...
exports.NvlinkTopologySource = NvlinkTopologySource;
function ingestNvlinkErrorCounter(prev, next, opts) {
    const meta = { semantic_type: 'counter', counter_width: 32 };
    return (0, counter_rate_transform_1.transformPair)(prev, next, meta, opts);
}
//# sourceMappingURL=nvlink-source.js.map