...
export const SAFE_T_ENVELOPE = Object.freeze({
  baseline: 'unknown-mean-integrated' as const,
  autocorrelation: 'ar1-whitened' as const,
...
export function safeTwoSampleTEValue(
  ...
  if (cal.len < 3) throw new RangeError(`safeTwoSampleTEValue: cal.len must be >= 3; got ${cal.len}`);
  if (test.len < 2) throw new RangeError(`safeTwoSampleTEValue: test.len must be >= 2; got ${test.len}`);
  if (test.start < 1) {
  ...
  if (cal.start < 0 || cal.start + cal.len > values.length || test.start + test.len > values.length) {
    throw new RangeError(
      `safeTwoSampleTEValue: window out of bounds (values.length=${values.length}, `
      + `cal=[${cal.start},${cal.start + cal.len}), test=[${test.start},${test.start + test.len}))`,
    );
  }
  for (let t = cal.start; t < cal.start + cal.len; t++) {
    if (!Number.isFinite(values[t])) throw new RangeError(`safeTwoSampleTEValue: non-finite value at calibration index ${t}`);
  }
  for (let t = test.start - 1; t < test.start + test.len; t++) {
    if (!Number.isFinite(values[t])) throw new RangeError(`safeTwoSampleTEValue: non-finite value at test index ${t}`);
  }

  const g = opts?.effectPriorVar ?? DEFAULT_EFFECT_PRIOR_VAR;
  if (!(g > 0)) throw new RangeError(`safeTwoSampleTEValue: effectPriorVar must be > 0; got ${g}`);
  ...
  const phi = opts?.ar1Phi ?? computePerSignalAr1Phi(calValues, mean(calValues));
  ...
  let ss = 0;
  for (const x of wc) ss += (x - mc) ** 2;
  for (const x of wt) ss += (x - mt) ** 2;
