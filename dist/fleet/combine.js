...
exports.combineProduct = combineProduct;
exports.combineProductUnguarded = combineProductUnguarded;
exports.combineMartingale = combineMartingale;
exports.adaptiveLambdas = adaptiveLambdas;
 ...
 */
function combineProduct(log_e_values, opts) {
    ...
    if (opts?.sequential !== true) {
        throw new Error('combineProduct: the product of e-values is an e-value only when they are independent or '
...
function combineProductUnguarded(log_e_values) {
...
function combineMartingale(log_e_values, lambdas) {
    if (log_e_values.length === 0) {
    ...
    for (let k = 0; k < log_e_values.length; k++) {
        const l = lambdas[k];
        if (!(l >= 0 && l <= 1))
            throw new Error(`combineMartingale: lambda[${k}] = ${l} outside [0, 1]`);
...
function adaptiveLambdas(log_e_values, gamma = 0.5) {
    if (!(gamma > 0 && gamma <= 1))
        throw new Error(`adaptiveLambdas: gamma must be in (0, 1], got ${gamma}`);
 ...
 */
function combineAverage(log_e_values) {
    if (log_e_values.length === 0) {
 ...
 */
function updateFleetEProcessState(state, log_fleet_e_t, log_threshold) {
