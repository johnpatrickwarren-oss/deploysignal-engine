...
exports.ONSET_MIXTURE_BOUNDED_ENVELOPE = exports.ONSET_MIXTURE_GAUSSIAN_ENVELOPE = exports.GEO_RHOS = void 0;
exports.supAdjuster = supAdjuster;
exports.normalizedMixtureEValue = normalizedMixtureEValue;
exports.geometricMixtureEValue = geometricMixtureEValue;
...
function normalizedMixtureEValue(r, inc = 'gaussian') {
    const T = r.length;
    if (T === 0)
        return 0;
    if (inc === 'gaussian') {
        let M = 0, mixPeak = 0;
        for (let t = 0; t < T; t++) {
            M = (1 + M) * (0, _bounded_bet_1.gInc)(r[t]);
            if (!isFinite(M))
                M = Number.MAX_VALUE;
    ...
    const K = _bounded_bet_1.BOUND_LAMBDAS.length;
    const M = new Array(K).fill(0);
    let mixPeak = 0;
    for (let t = 0; t < T; t++) {
        let mix = 0;
        for (let k = 0; k < K; k++) {
            M[k] = (1 + M[k]) * (0, _bounded_bet_1.gBounded)(r[t], _bounded_bet_1.BOUND_LAMBDAS[k]);
            if (!isFinite(M[k]))
                M[k] = Number.MAX_VALUE;
            mix += (M[k] + (T - 1 - t)) / T;
        }
        mix /= K;
        if (mix > mixPeak)
...
exports.GEO_RHOS = Object.freeze([1 / 64, 1 / 1024, 1 / 16384]);
 ...
 *  verbatim. */
function geometricMixtureEValue(r, inc = 'gaussian') {
    if (r.length === 0)
        return 0;
    const lams = inc === 'gaussian' ? [0] : _bounded_bet_1.BOUND_LAMBDAS; // gaussian: one per-tick-mixed increment
    const L = lams.length, K = exports.GEO_RHOS.length;
    const S = Array.from({ length: L }, () => new Array(K).fill(0));
    const w = exports.GEO_RHOS.map((rho) => rho); // w_t = ρ(1−ρ)^{t−1}, updated each tick
    const tail = new Array(K).fill(1); // (1−ρ)^t — weight of not-yet-started onsets
    let mixPeak = 0;
    for (let t = 0; t < r.length; t++) {
        let mix = 0;
        for (let l = 0; l < L; l++) {
            const g = inc === 'gaussian' ? (0, _bounded_bet_1.gInc)(r[t]) : (0, _bounded_bet_1.gBounded)(r[t], lams[l]);
            for (let k = 0; k < K; k++) {
                S[l][k] = g * (S[l][k] + w[k]);
                if (!isFinite(S[l][k]))
                    S[l][k] = Number.MAX_VALUE;
                mix += S[l][k] + tail[k];
            }
        }
        for (let k = 0; k < K; k++) {
            w[k] *= 1 - exports.GEO_RHOS[k];
            tail[k] *= 1 - exports.GEO_RHOS[k];
        }
        mix /= L * K;
        if (mix > mixPeak)
            mixPeak = mix;
...
exports.ONSET_MIXTURE_GAUSSIAN_ENVELOPE = Object.freeze({
    baseline: 'plug-in',
    autocorrelation: 'iid',
    null: 'mean-shift',
    variance: 'stable',
