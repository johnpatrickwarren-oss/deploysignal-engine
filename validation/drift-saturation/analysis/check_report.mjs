// validation/drift-saturation/analysis/check_report.mjs — pins REPORT.md to the run JSON by re-rendering and
// diffing (exit 1 on drift), then checks any `--expect <file>` of `selector = value` lines. Selectors:
// cells[cell,construction,level].field, p3[null,construction,level].field, p4[cell].field, manifest.field.
...
if (ei > 0) {
  const D = { cells: J('cells.json'), p3: J('p3.json'), p4: J('p4.json'), manifest };
  for (const raw of fs.readFileSync(process.argv[ei + 1], 'utf8').split('\n')) {
    ...
    let got;
    const sel = /^(cells|p3|p4)\[([^\]]+)\]\.(\w+)$/.exec(lhs);
    if (sel) {
      const k = sel[2].split(',').map((s) => s.trim());
      const hit = D[sel[1]].filter((c) => (sel[1] === 'cells' ? c.cell === k[0] && c.construction === k[1] && String(c.level) === k[2]
        : sel[1] === 'p3' ? c.null === k[0] && c.construction === k[1] && String(c.level) === k[2] : c.cell === k[0]));
