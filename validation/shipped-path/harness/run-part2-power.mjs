// harness/run-part2-power.mjs — C18 Part 2. The contamination power endpoint,
// re-run near the detection boundary.
//
// Identical to ../../contamination/harness/run-part-b.mjs except for the fault
// shape: a +0.75 sigma MEAN SHIFT from tick 100 (delta* from the effect-size
// sweep) instead of the x2 variance inflation, which saturated every arm at
// 1.0000 and made the endpoint discriminate nothing.
//
...
const FAULT_ONSET = 100, FAULT_SHIFT = 0.75;   // delta* from the effect-size sweep
    ...
    for (const mode of ['null', 'fault']) {
      ...
      for (let b = 0; b < BUNDLES && done < N; b++) {
        ...
        for (let i = 0; i < perBundle && done < N; i++, done++) {
          ...
          for (let t = 0; t < T; t++) {
            const z = draw();
            if (mode === 'fault' && t >= FAULT_ONSET) for (let k = 0; k < P; k++) z[k] += FAULT_SHIFT * CELL_SD;
            devs.push(z);
          }
      ...
      cells.push({
        ...
        mode_meaning: mode === 'null' ? 'false alarm (clean live stream)' : 'power (+0.75 sigma mean shift from tick 100)',
