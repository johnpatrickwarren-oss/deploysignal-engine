...
import { ValidityEnvelope, FdrPathAssertions, isValidForFdrPath } from '../detectors/validity-envelope';

/** Empirical breakdown fault fraction of the redescending robust common-mode (Tessera ADR 0015 ~20%). */
export const ROBUST_COMMON_MODE_BREAKDOWN = 0.2;
...
export function assembleFleetGuaranteeConditions(input: FleetGuaranteeInput): FleetGuaranteeConditions {
  const breakdownFraction = input.breakdownFraction ?? ROBUST_COMMON_MODE_BREAKDOWN;
  ...
  const eValueValidForFdr = isValidForFdrPath(input.eValueEnvelope, input.assertions);
  ...
  const fdrGuaranteedByConstruction =
    eValueValidForFdr && faultFractionUnderBreakdown && input.genuineCoupling && input.scalarCommonMode;
  const minDetectableEffect = input.minDetectableEffect ?? null;

  const unmet: string[] = [];
  if (!eValueValidForFdr) unmet.push('the per-shard e-value is INVALID under an estimated baseline (gate it or assert its regime)');
  if (!faultFractionUnderBreakdown) unmet.push(`fault fraction ${(input.faultFraction * 100).toFixed(0)}% ≥ breakdown ${(breakdownFraction * 100).toFixed(0)}%`);
  if (!input.genuineCoupling) unmet.push('the fleet lacks genuine common-mode coupling');
  if (!input.scalarCommonMode) unmet.push('the common-mode is not a per-tick scalar (heterogeneous loading)');
