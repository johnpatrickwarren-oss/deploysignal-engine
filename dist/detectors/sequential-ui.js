...
function buildNullSums(x) {
    const T = x.length;
    const s1 = new Float64Array(T), s1p = new Float64Array(T), s2 = new Float64Array(T), s2p = new Float64Array(T), s11 = new Float64Array(T);
    for (let s = 1; s < T; s++) {
        s1[s] = s1[s - 1] + x[s];
        s1p[s] = s1p[s - 1] + x[s - 1];
        s2[s] = s2[s - 1] + x[s] * x[s];
...
function nullLogLikAtPhi(ns, upTo, phi) {
    const n = upTo;
    if (n < 2)
        return 0;
...
function nullLogLikSup(ns, upTo) {
    if (upTo < 2)
        return 0;
    const GRID = 81;
...
function predictableFit(x, upTo, changeFrom, phiClip) {
    ...
    const m = (i) => (i >= changeFrom ? muPost : muPre);
 ...
 */
function sequentialUiMeanShiftEProcess(x, opts = {}) {
    if (x.length < 3)
        throw new RangeError(`sequential-ui: need ≥ 3 points, got ${x.length}`);
    const changeFrom = opts.changeFrom ?? 1;
    const phiClip = opts.phiClip ?? 0.95;
    const ns = buildNullSums(x);
    let numLog = 0;
    const logE = [];
    let firstCross01 = null;
...
//# sourceMappingURL=sequential-ui.js.map