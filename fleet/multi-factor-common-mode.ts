...
function robustSlope(x: ReadonlyArray<number>, y: ReadonlyArray<number>, c = 4.685): number {
  ...
  for (let it = 0; it < IRLS_MAX_ITER; it++) {
    const absRes: number[] = [];
    for (let i = 0; i < n; i++) absRes.push(Math.abs(y[i] - b * x[i]));
    ...
    if (d < 1e-9) break;
    const next = a / d;
    if (Math.abs(next - b) < IRLS_TOL) { b = next; break; }
    b = next;
  }
  return b;
}
...
function validateMatrix(X: ReadonlyArray<ReadonlyArray<number>>, calLen: number, fn: string): [number, number] {
  const n = X.length;
...
function fitOneFactor(D: number[][], n: number, t: number): number {
  const lam = new Array<number>(n).fill(1); // homogeneous start ⇒ first F̂ pass = the robust per-tick center
  const F = new Array<number>(t).fill(0);
  const col = new Array<number>(n);
  for (let pass = 0; pass < ALT_PASSES; pass++) {
    for (let j = 0; j < t; j++) {
      for (let i = 0; i < n; i++) col[i] = D[i][j];
      // First pass with λ≡1 is exactly the redescending per-tick center (PR B's robustLocation).
      F[j] = pass === 0 ? robustLocation(col) : robustSlope(lam, col);
    }
    for (let i = 0; i < n; i++) lam[i] = robustSlope(F, D[i]);
  }
  let energy = 0;
  for (let i = 0; i < n; i++) for (let j = 0; j < t; j++) { const c = lam[i] * F[j]; energy += c * c; D[i][j] -= c; }
  return energy;
}
...
export function multiFactorRobustResiduals(
  X: ReadonlyArray<ReadonlyArray<number>>,
  calLen: number,
  opts?: MultiFactorOptions,
): number[][] {
  ...
  const D: number[][] = X.map((row, i) => row.map((x) => x - lvl[i]));
  for (let k = 0; k < r; k++) fitOneFactor(D, n, t);
  return D;
}
...
export function factorDeflationEnergy(
  X: ReadonlyArray<ReadonlyArray<number>>,
  calLen: number,
  maxFactors: number,
): number[] {
  ...
  const D: number[][] = X.map((row, i) => row.map((x) => x - lvl[i]));
  let total = 0;
  for (let i = 0; i < n; i++) for (let j = 0; j < t; j++) total += D[i][j] * D[i][j];
  total = Math.max(total, 1e-12);
  const out: number[] = [];
  for (let k = 0; k < maxFactors; k++) out.push(fitOneFactor(D, n, t) / total);
  return out;
}
