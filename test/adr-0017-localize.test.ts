...
import { localizeFaults } from '../fleet/localize';
...
function genFleet(seed: number, victim: number): number[][] {
  const rng = lcg(seed); const g = (): number => gaussian(rng);
  const mkFactor = (rampPerT: number): number[] => {
    const f = new Array(T).fill(0); let p = g();
  ...
  const X: number[][] = [];
  for (let i = 0; i < N; i++) {
    const lvl = 5 * g(); const lamCool = 0.4 + 1.2 * rng(), lamPower = 0.4 + 1.2 * rng();
...
const factorPartitions = (): number[][] => [
  Array.from({ length: N }, (_, i) => coolOf(i)),
  Array.from({ length: N }, (_, i) => powerOf(i)),
];
const racks = (): number[] => Array.from({ length: N }, (_, i) => rackOf(i));
const cal = { start: 0, len: 60 }, tst = { start: FONSET, len: NT };

test('localize: flags a single-shard fault FAR more often than a healthy shard (signal ≫ noise)', () => {
  ...
  for (let seed = 1; seed <= 3; seed++) {
    for (const victim of victims) {
      ...
      if (r.selected.includes(victim)) hits++;
      instances++;
      for (let i = 0; i < N; i++) { if (i === victim) continue; healthyTot++; if (r.selected.includes(i)) healthySel++; }
    }
  }
  ...
  assert.ok(detRate >= 0.6, `should localise the fault most of the time; got ${(detRate * 100).toFixed(0)}%`);
  assert.ok(detRate >= 2 * fpRate, `detection should be ≫ healthy FP rate; det=${(detRate * 100).toFixed(0)}% fp=${(fpRate * 100).toFixed(1)}%`);
});

test('localize: result structure is consistent', () => {
  const X = genFleet(3, 25);
  const r = localizeFaults({
    X, referenceLen: REF, cal, test: tst,
    factorPartitions: factorPartitions(), localizationGroups: racks(), qLevel: 0.1,
  });
  assert.equal(r.perShardEValue.length, N);
  assert.ok(r.perShardEValue.every((e) => Number.isFinite(e) && e >= 0));
  const fromGroups = [...r.byGroup.values()].flat().sort((a, b) => a - b);
  assert.deepEqual(r.selected, fromGroups);
  const grp = racks();
  for (const [g, idxs] of r.byGroup) for (const i of idxs) assert.equal(grp[i], g);
});

test('localize: guards', () => {
  const X = genFleet(1, -1);
  const P = factorPartitions(), G = racks();
  assert.throws(() => localizeFaults({ X: [], referenceLen: REF, cal, test: tst, factorPartitions: P, localizationGroups: G, qLevel: 0.1 }), /at least one shard/);
  assert.throws(() => localizeFaults({ X, referenceLen: REF, cal, test: tst, factorPartitions: P, localizationGroups: G, qLevel: 0 }), /qLevel must be in/);
  assert.throws(() => localizeFaults({ X, referenceLen: REF, cal, test: tst, factorPartitions: P, localizationGroups: [0, 1, 2], qLevel: 0.1 }), /one label per shard/);
});
