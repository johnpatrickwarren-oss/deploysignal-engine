// harness/run-rffdim.mjs — PREREGISTRATION §7, A-P5. Is the zero power of
// A-P3 structural, or just 256 random Fourier features being too few?
//
...
const DIMS = [256, 1024, 4096];

const trace = (S) => S.reduce((a, r, i) => a + r[i], 0);

function restamp(famC, k, dim) {
  const bp = famC.betting_e_process_params;
  if (dim) bp.rff_dim = dim;                 // runtime regenerates the map from this
...
for (const sigma of ['corr', 'diag']) {
  ...
  for (const dim of DIMS) {
    for (const arm of ['power', 'healthy']) {
      const maxLogs = [], ks = [];
      let done = 0;
      for (let b = 0; b < BUNDLES && done < N; b++) {
        ...
        for (let i = 0; i < perBundle && done < N; i++, done++) {
          ...
          const devs = [];
          for (let t = 0; t < T; t++) {
            devs.push(arm === 'power' && t >= FAULT_ONSET ? postDraw() : preDraw());
          }
          maxLogs.push(runFamilyC(cfg, famC, devs).maxLog);
        }
      }
      const fires = maxLogs.filter((x) => x >= Math.log(20)).length;
      cells.push({ arm, sigma, rff_dim: dim, k_mean: ks.reduce((a, b) => a + b, 0) / ks.length,
        n: done, ticks: T, alpha: 0.05, fires, rate: fires / done,
        mode: MODE, engine_version: engineVersion, git_sha: gitSha });
      process.stderr.write(`${arm.padEnd(8)} ${sigma} D=${String(dim).padStart(4)} `
        + `rate=${(fires / done).toFixed(4)} (${((Date.now() - t0) / 1000).toFixed(0)}s)\n`);
    }
  }
}

const outDir = path.join(STUDY, 'results', MODE === 'live' ? 'live' : 'sim',
  `rffdim-${new Date().toISOString().replace(/[-:.]/g, '').slice(0, 15)}Z`);
fs.mkdirSync(outDir, { recursive: true });
fs.writeFileSync(path.join(outDir, 'summary.json'), `${JSON.stringify({ cells }, null, 1)}\n`);
fs.writeFileSync(path.join(outDir, 'manifest.json'), `${JSON.stringify({
  study: 'bandwidth-scale-rffdim', prereg: '../PREREGISTRATION.md §7 A-P5', dims: DIMS,
