...
| (this freeze) | `2bbc557` | **The mixture card's note corrected, and the first re-freeze since v0.6.11-pre: every source sha moved with the code is stated.** `family_A_mixture_supermartingale`'s `notes` no longer says "NaN on right-skewed (N5)" — that reading was a detector-audit harness defect (every N5 draw NaN; h0-battery Amendment A7 Correction 2, 2026-09-25) — and names A7's DIVERGENT readings on t3 and the lognormal. No other `guarantee`, `regime`, `falsifier`, `shipped_path`, `budget`, `prior_evidence`, `class`, `aliases` or `detector_id` field changed on any card. **23 `source_files[].sha256` moved on 13 cards**, the collateral of the whole-directory route across every engine release from v0.6.11-pre to v0.11.0-pre (ADR 0029 e-SR, ADR 0031, ADR 0032, ADR 0033, ADR 0034, ADR 0035, A7): e_sr_mean_shift (`detectors/e-sr-mean-shift.ts`, `detectors/validity-envelope.ts`); e_sr_mean_shift_bounded (`detectors/e-sr-mean-shift.ts`, `detectors/validity-envelope.ts`, `fleet/calibration-monitor.ts`); family_A_betting_e_process (`detectors/betting-e-process.ts`, `detectors/validity-envelope.ts`); family_A_mixture_supermartingale (`detectors/_page-cusum-mixture.ts`, `detectors/family-a-mixture-supermartingale.ts`, `detectors/validity-envelope.ts`); family_C_safe_hotelling (`detectors/validity-envelope.ts`); family_D_spectral_e_detector (`detectors/spectral.ts`, `detectors/validity-envelope.ts`); family_E_conformal (`detectors/conformal.ts`, `detectors/validity-envelope.ts`); family_E_conformal_heldout (`detectors/conformal.ts`); group_average_e_value (`detectors/group-average-e-value.ts`); safe_t_e_value (`detectors/validity-envelope.ts`); sequential_mmd_betting_e_process (`detectors/sequential-mmd.ts`, `detectors/family-c-betting-e-process.ts`, `detectors/validity-envelope.ts`); sequential_ui_e_process (`detectors/validity-envelope.ts`); universal_inference_e_value (`detectors/validity-envelope.ts`). `detectors/validity-envelope.ts` moves on 11 cards (the φ bound, the e-detector statistic, ADR 0035's `tailPremise` and its assertions, A7's premises). All 17 cards move `engine_pin.sha` to `2bbc557` and `engine_pin.version` 0.6.11-pre → 0.11.0-pre. **Re-score `run-20260926T035435Z` at the new pins: all 17 verdicts, tiers and stage tokens identical to `run-20260925T174544Z`**; `expiry-check` reports no drift at this pin. A dedicated commit carrying nothing else. |
...
| (this freeze) | `88b1b75` | **Pins plus ONE new card, disclosed here.** Amendment v1.C69 edits `lib/score.mjs` and `verdict.mjs` (the e_detector class branch; `lib/guards.mjs` and `lib/constants.mjs` also change but no v1-class card pins them — a gap the amendment records), so the nine cards that pin those two files moved those two `source_files[].sha256`; the other six moved **only `engine_pin.sha`** (all fifteen `c6d46f8`/`72af438` → the freeze's pin). **The sixteenth card, `e_sr_mean_shift.json`, joins**, frozen for the first time (class `e_detector`, pins its module, the envelope, all of `lib/`'s scoring files and its own harness). No `guarantee`, `regime`, `falsifier`, `shipped_path`, `budget`, `prior_evidence`, `class`, `aliases` or `detector_id` field of any of the fifteen changed — audited field by field before the commit. **No verdict moves:** the fifteen card JSONs of `run-20260903T214235Z` are byte-identical to `run-20260902T230123Z`'s outside the pin fields (checked mechanically, 15 of 15); the new card scores USE T1 (S1 MISSING, S2 PASS with the two first-run INCONCLUSIVE cells named as gaps, S3 PASS, S4 PASS), the golden row registered before the run. |
...
| (this freeze) | `68b9b2c` | **Pins plus ONE new card, disclosed here (the C69 shape).** Amendment v1.C77 edits `lib/constants.mjs` (the bounded growth function; `eDetectorDelayBound` gains a third argument defaulting to `'gaussian'`) and `lib/score.mjs` (one line: the floor at the card's declared increment), so the nine v1-class cards that pin `score.mjs` moved that pin and every card moved `engine_pin.sha`; `e_sr_mean_shift` also moved its `lib/constants.mjs` pin (the only v1 card pinning it). **The seventeenth card, `e_sr_mean_shift_bounded.json`, joins**, frozen for the first time — class `e_detector`, `guarantee.regime.increment: 'bounded'`, N5/N6/N8 inside `null_prefixes` — and scores **REFUSE** (S2 REFUTED on N5 at 898 ± 4 and on N2-m30/m100/m500 and N4-p06/p09 at 437–863; S3 PASS, every canonical cell within the bounded D*; S4 PASS): the pre-run golden row said USE, the pre-run Amendment A1 revised it to REFUSE after an unscored pilot, and the golden test carries the measured row with the prediction named as wrong. No `guarantee`, `regime`, `falsifier`, `shipped_path`, `budget`, `prior_evidence`, `class`, `aliases` or `detector_id` field of any of the sixteen changed. **No verdict moves:** the sixteen card JSONs of `run-20260905T052027Z` are byte-identical to `run-20260903T214235Z`'s outside the pin fields, checked by `tools/compare-card-runs.mjs` (16 of 16). `engine_pin.version` is `0.6.11-pre` on all seventeen. |
...
## The `e_detector` class (Amendment v1.C69, 2026-09-03)

The fourth class, for run-length guarantees (`E∞[N*] ≥ 1/α_ARL`, not `E[e] ≤ 1`). Its S2 and S3
instruments are the two v1.C66 endpoints, which for THIS class carry verdict authority:
`lib/guards.mjs`'s `arlRule` clears a cell on `arl0_T − 1.645·se ≥ 1/alpha_arl`, refutes on
`arl0_T + 1.645·se < 1/alpha_arl` and records INCONCLUSIVE between as missing evidence (the
scorer recomputes the token from the fields; a disagreeing recorded token voids the run); S3 adds
a delay floor on every in-regime cell at the class canonical severity (K1, 1.5σ):
`delay_canonical + 1.645·se ≤ D*(alpha_arl, delta_eff)`, `D*` the Theorem 4.3 + Prop. B.2 bound
(`lib/constants.mjs` `eDetectorDelayBound`) at the effective whitened shift
`delta·sqrt((1−φ)/(1+φ))` (`effectiveShift`), with a new S3 status SLOW that maps to ADVISORY.
S4 is inverted for the class: PASS iff the budget is advisory and the id is NOT wired into
