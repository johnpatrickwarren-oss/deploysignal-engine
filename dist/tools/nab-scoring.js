...
exports.NAB_PROFILES = void 0;
exports.computeNABScore = computeNABScore;
exports.aggregateFamilyScore = aggregateFamilyScore;
 ...
 */
function computeNABScore(firings, annotations, profile) {
    ...
    const claimedFiringIdx = new Set();
    for (const w of wins) {
        const width = Math.max(1, w.end - w.start);
        // Find first firing within [w.start, w.end].
        const inWindowIdx = firings.findIndex((f, idx) => f.fire && !claimedFiringIdx.has(idx) && f.tick >= w.start && f.tick <= w.end);
        if (inWindowIdx >= 0) {
    ...
    if (perfect <= 0)
        return Math.max(0, raw); // Defensive.
...
function aggregateFamilyScore(perDatasetScores) {
    const values = Object.values(perDatasetScores);
    if (values.length === 0)
        return 0;
    return values.reduce((a, b) => a + b, 0) / values.length;
}
//# sourceMappingURL=nab-scoring.js.map