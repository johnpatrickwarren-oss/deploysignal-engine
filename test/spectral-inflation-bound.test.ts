...
import type { FamilyDPerSignal, SpectralInflationBound } from '../types/families/d';
...
const thresholdWith = (bound?: SpectralInflationBound): number => {
...
test('the measurement form prices the threshold identically to its bare c', () => {
  ...
  assert.ok(Math.abs(thresholdWith(measured) - thresholdWith(1.1076)) < 1e-12);
});
