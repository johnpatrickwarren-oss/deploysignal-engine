...
test('n < 3 returns exactly 0 — the minimum-samples floor', () => {
  ...
  for (const values of [[], [1], [1, 2]]) {
    assert.equal(
      computePerSignalAr1Phi(values, 0),
      0,
      `n=${values.length} must short-circuit before the OLS ratio`,
    );
  }
});

test('a constant window returns 0 via the variance guard, not NaN', () => {
  ...
  const phi = computePerSignalAr1Phi([5, 5, 5, 5, 5, 5], 5);
  assert.equal(phi, 0);
  assert.ok(Number.isFinite(phi), 'must not be NaN');
});

test('with zero OLS correlation the result IS the correction term, 1/n', () => {
  ...
  for (const n of [3, 10, 100]) {
    ...
    assert.ok(
      Math.abs(computePerSignalAr1Phi(values, 0) - 1 / n) < 1e-12,
      `n=${n}: expected the bare correction 1/n = ${1 / n}`,
    );
  }
});

test('the correction is ~33x larger at the n=3 boundary than at n=100', () => {
  ...
  const at = (n: number) => computePerSignalAr1Phi([1, ...new Array(n - 1).fill(0)], 0);
  const ratio = at(3) / at(100);
  assert.ok(
    Math.abs(ratio - 100 / 3) < 1e-9,
    `expected the correction to scale as 1/n (ratio ${100 / 3}); got ${ratio}`,
  );
  assert.ok(at(3) > 0.3, `n=3 correction should be material; got ${at(3)}`);
});

test('n=3 is the first length that estimates rather than floors', () => {
  ...
  assert.equal(computePerSignalAr1Phi([1, 0], 0), 0);
  assert.ok(computePerSignalAr1Phi([1, 0, 0], 0) > 0);
});
