...
exports.InMemoryTopologyAuditEmitter = exports.TopologyEnricher = exports.defaultDeployNodeResolver = exports.OtelServiceGraphV1 = exports.StaticTopologySource = exports.OTEL_SERVICE_GRAPH_V1_ID = void 0;
exports.pureJsSha256 = pureJsSha256;
exports.computeSnapshotHash = computeSnapshotHash;
exports.projectToAuditEvent = projectToAuditEvent;
...
exports.OTEL_SERVICE_GRAPH_V1_ID = 'otel_service_graph_v1';
...
exports.StaticTopologySource = StaticTopologySource;
...
class OtelServiceGraphV1 {
    constructor(ref) {
        this.id = exports.OTEL_SERVICE_GRAPH_V1_ID;
        this.version = 'v1.0';
        this.cached = null;
        this.ref = ref;
    }
    async fetchSnapshot(ctx) {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(`TOPOLOGY_FETCH_HTTP_${res.status}`);
...
exports.OtelServiceGraphV1 = OtelServiceGraphV1;
const defaultDeployNodeResolver = (deploy_id, snapshot) => {
    for (const n of snapshot.nodes)
        if (n.metadata?.deploy_id === deploy_id)
            return n.id;
    for (const n of snapshot.nodes)
        if (n.id === deploy_id)
            return n.id;
    for (const n of snapshot.nodes)
        if (n.service_name === deploy_id)
            return n.id;
    return null;
};
exports.defaultDeployNodeResolver = defaultDeployNodeResolver;
...
class TopologyEnricher {
    ...
    rankCandidates(group, snapshot, events, hopByNode) {
        ...
        for (const ev of events) {
            ...
            if (hop === undefined)
                continue; // beyond maxHop or unknown node
            const overlap = this.temporalOverlap(group, ev);
            if (overlap <= 0)
                continue; // D4-adjacent: zero-overlap dropped
    ...
    temporalOverlap(group, ev) {
        const gs = group.window_start_ts;
        const ge = group.window_end_ts;
        if (ev.event_window_start_ts !== undefined &&
...
exports.InMemoryTopologyAuditEmitter = InMemoryTopologyAuditEmitter;
...
function projectToAuditEvent(result) {
    return {
...
//# sourceMappingURL=topology-overlay.js.map