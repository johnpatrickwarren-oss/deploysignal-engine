...
exports.REGISTERED_GEOMETRY_K6SLOW = exports.LOG_WEALTH_FLOOR_K6SLOW = exports.LOG_WEALTH_THRESHOLD_K6SLOW = exports.ALPHA_K6SLOW = exports.KAPPA_K6SLOW = exports.N_ROWS_K6SLOW = exports.M_K6SLOW = exports.N_A_K6SLOW = exports.W_K6SLOW = void 0;
exports.calibrateEcdfAccumulator = calibrateEcdfAccumulator;
exports.ecdfAccumulatorWindow = ecdfAccumulatorWindow;
exports.ecdfAccumulatorWealth = ecdfAccumulatorWealth;
exports.nullGrowthScreen = nullGrowthScreen;
const _wealth_1 = require("./_wealth");
/** Live/reference window length. FROZEN (v2.K6A.1 K6A.1.2). */
exports.W_K6SLOW = 150;
/** Reference-segment length: A = rows 1..25,000 of the 100,000-row draw. FROZEN. */
exports.N_A_K6SLOW = 25000;
...
exports.M_K6SLOW = 500;
/** Calibration substrate rows per cell: 25,000 + 500*150. FROZEN. */
exports.N_ROWS_K6SLOW = exports.N_A_K6SLOW + exports.M_K6SLOW * exports.W_K6SLOW;
...
exports.LOG_WEALTH_FLOOR_K6SLOW = Math.log(1e-12);
exports.REGISTERED_GEOMETRY_K6SLOW = Object.freeze({
    W: exports.W_K6SLOW,
    nA: exports.N_A_K6SLOW,
    m: exports.M_K6SLOW,
});
...
function assertGeometry(fn, geom) {
    if (!(Number.isInteger(geom.W) && geom.W > 0)) {
        throw new Error(`${fn}: W must be a positive integer (K6A.1.2 freezes W = ${exports.W_K6SLOW}), got ${geom.W}`);
    }
    if (!(Number.isInteger(geom.nA) && geom.nA > 0)) {
        throw new Error(`${fn}: nA must be a positive integer (K6A.1.2 freezes A = ${exports.N_A_K6SLOW}), got ${geom.nA}`);
    }
    if (!(Number.isInteger(geom.m) && geom.m > 0)) {
        throw new Error(`${fn}: m must be a positive integer (K6A.1.2 freezes m = ${exports.M_K6SLOW}), got ${geom.m}`);
    }
}
...
function crossSum(x, sortedA, prefixA) {
...
function pairSum(sortedWindow) {
    const W = sortedWindow.length;
    let s = 0;
    for (let r = 1; r <= W; r++)
        s += (2 * r - W - 1) * sortedWindow[r - 1];
    return 2 * s;
}
/** K6A.2's `energy` form, assembled from the two helpers above. */
function energyT(window, sortedA, prefixA) {
    const W = window.length;
    const nA = sortedA.length;
    let cross = 0;
    for (const x of window)
        cross += crossSum(x, sortedA, prefixA);
...
function fingerprint(geom, sortedBlockT) {
...
function calibrateEcdfAccumulator(rows, geom = exports.REGISTERED_GEOMETRY_K6SLOW) {
    const fn = 'calibrateEcdfAccumulator';
    assertGeometry(fn, geom);
    const { W, nA, m } = geom;
    if (rows.length <= nA) {
        throw new Error(`${fn}: need more than nA = ${nA} rows to split A and B (K6A.1.9: A = rows 1..${nA}, `
            + `B = rows ${nA + 1}..${nA + m * W}), got ${rows.length}`);
    }
    const bLen = rows.length - nA;
    if (bLen % W !== 0) {
        throw new Error(`${fn}: B must be an exact multiple of W = ${W} (K6A.1.2: no remainder, the split is exact `
            + `and not floor-based), got ${bLen} rows after A = ${nA}`);
    }
    const mSplit = bLen / W;
    if (mSplit !== m) {
        throw new Error(`${fn}: m must be ${m}, got ${mSplit} from ${rows.length} rows `
            + `(K6A.1.2: n = ${nA + m * W} = A ${nA} + B ${m * W} -> ${m} blocks of ${W})`);
    }
    for (let i = 0; i < rows.length; i++) {
        if (!Number.isFinite(rows[i])) {
            throw new Error(`${fn}: non-finite row at index ${i} (${rows[i]}) — the substrate is a continuous `
    ...
    if (!(sortedA[nA - 1] > sortedA[0])) {
        throw new Error(`${fn}: degenerate reference ECDF Fhat_A (A is a point mass, min = max = ${sortedA[0]}) — `
            + 'every energy distance would then be a function of |x - a| alone and the rank carries no '
            + 'distributional evidence');
    }
    const prefixA = new Array(nA + 1);
    prefixA[0] = 0;
    for (let i = 0; i < nA; i++)
        prefixA[i + 1] = prefixA[i] + sortedA[i];
    const blockT = new Array(m);
    for (let j = 0; j < m; j++) {
        ...
        if (!(bmax > bmin)) {
            throw new Error(`${fn}: constant reference block B_${j + 1} (all ${W} rows = ${bmin}) — the block-`
    ...
    if (!(sortedBlockT[m - 1] > sortedBlockT[0])) {
        throw new Error(`${fn}: zero spread across the ${m} reference block statistics (every T(B_j) = `
...
function ecdfAccumulatorWindow(window, cal, kappa = exports.KAPPA_K6SLOW) {
    assertKappaInUnitInterval('ecdfAccumulatorWindow', kappa);
    if (window.length !== cal.W) {
        throw new Error(`ecdfAccumulatorWindow: window.length must be ${cal.W}, got ${window.length} `
            + '(K6A.1.9: the live stream is consumed in DISJOINT W-blocks with no remainder)');
    }
    for (let i = 0; i < window.length; i++) {
        if (!Number.isFinite(window[i])) {
            throw new Error(`ecdfAccumulatorWindow: non-finite value at window index ${i} (${window[i]}) — `
...
function ecdfAccumulatorWealth(windows, cal) {
    let logM = 0; // log(1) — wealth starts at 1
    const log = [];
    let crossingIndex = -1;
    for (let i = 0; i < windows.length; i++) {
        const { e } = ecdfAccumulatorWindow(windows[i], cal);
        logM = (0, _wealth_1.advanceLogWealth)(logM, Math.log(e), exports.LOG_WEALTH_FLOOR_K6SLOW);
        log.push(logM);
        if (crossingIndex === -1 && logM >= exports.LOG_WEALTH_THRESHOLD_K6SLOW)
            crossingIndex = i;
    }
    return { wealth: (0, _wealth_1.wealthView)(logM), log, crossingIndex };
}
...
function nullGrowthScreen(cal, opts) {
    const fn = 'nullGrowthScreen';
    const kappa = opts.kappa === undefined ? exports.KAPPA_K6SLOW : opts.kappa;
...
//# sourceMappingURL=shape-ecdf-accumulator.js.map