...
exports.sampleAutocovariance = sampleAutocovariance;
exports.autocovarianceSequence = autocovarianceSequence;
exports.yuleWalkerLevinson = yuleWalkerLevinson;
exports.fitArP = fitArP;
exports.prewhitenAr = prewhitenAr;
...
function sampleAutocovariance(x, mean, k) {
    const N = x.length;
    if (k < 0 || k >= N)
        return 0;
    let s = 0;
...
function autocovarianceSequence(x, mean, p) {
    const out = new Array(p + 1);
    for (let k = 0; k <= p; k++)
        out[k] = sampleAutocovariance(x, mean, k);
    return out;
}
 ...
 *  a lower order. */
function yuleWalkerLevinson(autocovariances) {
    const p = autocovariances.length - 1;
    if (p < 0)
        throw new Error('yuleWalkerLevinson: need at least γ̂_0');
    if (p === 0) {
    ...
    const reflectionCoefficients = new Array(p).fill(0);
    let sigma2 = gamma[0];
    if (!(sigma2 > 0)) {
        // Degenerate: zero variance. Return all-zero AR coefficients.
        return { phi: new Array(p).fill(0), sigma2_innovation: 0, reflection_coefficients: reflectionCoefficients };
    }
    for (let m = 1; m <= p; m++) {
        ...
        const updated = new Array(m).fill(0);
        for (let i = 1; i < m; i++)
            updated[i - 1] = prev[i - 1] - k_m * prev[m - 1 - i];
        updated[m - 1] = k_m;
        for (let i = 0; i < m; i++)
            phi[i] = updated[i];
        sigma2 = sigma2 * (1 - k_m * k_m);
        if (sigma2 < 0)
            sigma2 = 0; // Numerical floor; |k_j| > 1 → non-stationary fit
    }
...
function fitArP(values, mean, options) {
    ...
    const icTrace = [];
    let best = { p: 0, phi: [], sigma2: gamma[0], icValue: Number.POSITIVE_INFINITY, k: [] };
...
function prewhitenAr(values, mean, phi) {
    const p = phi.length;
    if (p === 0)
        return values.slice();
    const N = values.length;
