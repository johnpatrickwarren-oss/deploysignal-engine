...
exports.eBHConditionalCalibration = eBHConditionalCalibration;
const e_bh_1 = require("./e-bh");
/** |R̂_j(e with e_j := x)| = |e-BH(·).selected ∪ {j}|. */
function rHatSize(eValues, j, x, qLevel) {
    const prev = eValues[j];
    eValues[j] = x;
    const sel = (0, e_bh_1.eBenjaminiHochberg)(eValues, qLevel).selected;
    eValues[j] = prev;
...
function eBHConditionalCalibration(eValues, qLevel, nullSurvival, opts) {
    const m = eValues.length;
    if (m === 0)
        throw new Error('eBHConditionalCalibration: empty input array');
    if (!(qLevel > 0 && qLevel <= 1))
        throw new RangeError(`eBHConditionalCalibration: qLevel must be in (0,1]; got ${qLevel}`);
    ...
    if (!(nullMean > 0 && nullMean <= 1))
        throw new RangeError(`eBHConditionalCalibration: nullMean must be in (0,1]; got ${nullMean}`);
    const work = eValues.slice(); // scratch for the leave-one-in recomputation
    const boosted = new Array(m);
    for (let j = 0; j < m; j++) {
    ...
    return (0, e_bh_1.eBenjaminiHochberg)(boosted, qLevel);
}
//# sourceMappingURL=e-bh-conditional-calibration.js.map