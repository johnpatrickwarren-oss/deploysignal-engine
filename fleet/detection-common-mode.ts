...
function validate(
  ...
  if (partitions.length === 0) throw new RangeError(`${fn}: partitions must have at least one factor partition`);
...
function domainMembers(part: ReadonlyArray<number>, n: number): Map<number, number[]> {
  const m = new Map<number, number[]>();
  for (let i = 0; i < n; i++) {
    const d = part[i];
    if (d < 0) continue;
    const cur = m.get(d);
    if (cur) cur.push(i); else m.set(d, [i]);
  }
  return m;
}
...
export function detectionOrientedResiduals(
  ...
  calLen: number,
  partitions: ReadonlyArray<ReadonlyArray<number>>,
  opts?: DetectionCommonModeOptions,
): number[][] {
  ...
  const R: number[][] = X.map((row) => {
    const lvl = median(row.slice(0, calLen));
