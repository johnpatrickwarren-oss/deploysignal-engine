...
exports.E_SR_MEAN_SHIFT_BOUNDED_ENVELOPE = exports.E_SR_MEAN_SHIFT_ENVELOPE = exports.E_SR_BOUNDED_LAMBDA_GRID = exports.E_SR_DEFAULT_ALPHA_ARL = exports.E_SR_LAMBDA_GRID = void 0;
exports.eSrLambdaGrid = eSrLambdaGrid;
...
function eSrLambdaGrid(params = {}) {
    ...
    const grid = params.lambdas ?? (inc === 'bounded' ? exports.E_SR_BOUNDED_LAMBDA_GRID : exports.E_SR_LAMBDA_GRID);
...
function standardizeAr1Residual(x, x_prev, mu, sigma, phi = 0) {
    if (!(sigma > 0))
        throw new Error(`standardizeAr1Residual: sigma must be > 0, got ${sigma}`);
    if (!(Math.abs(phi) < 1))
        throw new Error(`standardizeAr1Residual: |phi| must be < 1, got ${phi}`);
...
function evaluateESrMeanShift(r, params, state) {
    if (!Number.isFinite(r))
        throw new Error(`evaluateESrMeanShift: r must be finite, got ${r}`);
    const alpha = params.alpha_arl ?? exports.E_SR_DEFAULT_ALPHA_ARL;
    if (!(alpha > 0 && alpha < 1))
        throw new Error(`evaluateESrMeanShift: alpha_arl must be in (0,1), got ${alpha}`);
    const inc = params.increment ?? 'gaussian';
    if (inc !== 'gaussian' && inc !== 'bounded')
        throw new Error(`evaluateESrMeanShift: increment must be 'gaussian' or 'bounded', got ${String(inc)}`);
    const lambdas = eSrLambdaGrid(params);
    const K = lambdas.length;
    if (state.log_M_sr.length !== K)
        throw new Error(`evaluateESrMeanShift: state has ${state.log_M_sr.length} components, grid has ${K}`);
    const t = state.t;
...
exports.E_SR_MEAN_SHIFT_ENVELOPE = Object.freeze({
    baseline: 'plug-in',
...
exports.E_SR_MEAN_SHIFT_BOUNDED_ENVELOPE = Object.freeze({
    baseline: 'plug-in',
    autocorrelation: 'ar1-whitened',
    null: 'mean-shift',
    variance: 'stable',
...
//# sourceMappingURL=e-sr-mean-shift.js.map