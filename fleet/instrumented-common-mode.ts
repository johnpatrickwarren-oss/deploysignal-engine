...
import { median } from './multi-factor-common-mode';
...
function solveRidge(A: number[][], b: number[], ridge: number): number[] | null {
  const k = b.length;
...
export function instrumentedCommonModeResiduals(
  X: ReadonlyArray<ReadonlyArray<number>>,
  calLen: number,
  factorSignals: ReadonlyArray<ReadonlyArray<number>>,
  membership: ReadonlyArray<ReadonlyArray<number>>,
  opts?: InstrumentedCommonModeOptions,
): number[][] {
  const fn = 'instrumentedCommonModeResiduals';
  const n = X.length;
  if (n === 0) throw new RangeError(`${fn}: X must have at least one shard`);
  const t = X[0].length;
  if (t === 0) throw new RangeError(`${fn}: shard rows must be non-empty`);
  for (let i = 0; i < n; i++) {
    if (X[i].length !== t) throw new RangeError(`${fn}: ragged matrix — row ${i} has length ${X[i].length}, expected ${t}`);
    for (let j = 0; j < t; j++) if (!Number.isFinite(X[i][j])) throw new RangeError(`${fn}: non-finite value at [${i}][${j}]`);
  }
  ...
  for (let k = 0; k < nf; k++) {
    if (factorSignals[k].length !== t) throw new RangeError(`${fn}: factor ${k} has length ${factorSignals[k].length}, expected ${t}`);
    for (let j = 0; j < t; j++) if (!Number.isFinite(factorSignals[k][j])) throw new RangeError(`${fn}: non-finite factor value at [${k}][${j}]`);
  }
  ...
  if (!(ridge >= 0)) throw new RangeError(`${fn}: ridge must be >= 0; got ${ridge}`);

  // Centre each factor on the reference window (load-bearing — an un-centred fit biases the loadings).
  const cf: number[][] = factorSignals.map((f) => {
    const c = median(f.slice(0, calLen));
    return f.map((v) => v - c);
  });

  const R: number[][] = new Array(n);
  for (let i = 0; i < n; i++) {
    const lvl = median(X[i].slice(0, calLen));
    const row = X[i].map((v) => v - lvl);
    const idx = membership[i];
    if (idx.length === 0) { R[i] = row; continue; }
    const regs: number[][] = new Array(idx.length);
    ...
    for (let a = 0; a < k; a++) {
      for (let c = a; c < k; c++) {
        let z = 0;
        for (let j = 0; j < calLen; j++) z += regs[a][j] * regs[c][j];
        A[a][c] = z; A[c][a] = z;
      }
      let zy = 0;
      for (let j = 0; j < calLen; j++) zy += regs[a][j] * row[j];
      b[a] = zy;
    }
    ...
    if (lam) {
      for (let j = 0; j < t; j++) {
        let cm = 0;
        for (let a = 0; a < k; a++) cm += lam[a] * regs[a][j];
        row[j] -= cm;
      }
