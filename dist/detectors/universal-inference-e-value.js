...
exports.UI_MEAN_SHIFT_ENVELOPE = void 0;
exports.universalInferenceMeanShiftEValue = universalInferenceMeanShiftEValue;
/** Validity envelope for the universal-inference mean-shift e-value (ADR 0010). */
exports.UI_MEAN_SHIFT_ENVELOPE = Object.freeze({
    baseline: 'unknown-mean-mle',
    autocorrelation: 'ar1-any-phi', // valid for ANY φ by construction (incl. near unit root)
    null: 'mean-shift',
...
function segmentLogLik(values, seg, mu, phi, s2) {
    let ll = 0;
    let prev = seg.pred;
...
function fitGivenPhi(values, segs, phi, sepMeans) {
    const mus = [];
    ...
    for (let k = 0; k < segs.length; k++) {
        ...
        for (let i = segs[k].i0; i < segs[k].i1; i++) {
            const e = values[i] - (mus[k] + phi * (prev - mus[k]));
            rss += e * e;
            m++;
            prev = values[i];
        }
    ...
    let ll = 0;
    for (let k = 0; k < segs.length; k++)
        ll += segmentLogLik(values, segs[k], mus[k], phi, s2);
...
function fitAR1(values, segs, sepMeans) {
...
function universalInferenceMeanShiftEValue(values, cal, test) {
    ...
    if (cal.len < 6)
        throw new RangeError(`universalInferenceMeanShiftEValue: cal.len must be >= 6; got ${cal.len}`);
    if (test.len < 6)
        throw new RangeError(`universalInferenceMeanShiftEValue: test.len must be >= 6; got ${test.len}`);
    if (test.start < 1) {
    ...
    if (cal.start < 0 || cal.start + cal.len > values.length || test.start + test.len > values.length) {
        throw new RangeError(`universalInferenceMeanShiftEValue: window out of bounds (values.length=${values.length}, `
            + `cal=[${cal.start},${cal.start + cal.len}), test=[${test.start},${test.start + test.len}))`);
    }
    for (let i = cal.start; i < cal.start + cal.len; i++) {
        if (!Number.isFinite(values[i]))
            throw new RangeError(`universalInferenceMeanShiftEValue: non-finite value at calibration index ${i}`);
    }
    for (let i = test.start - 1; i < test.start + test.len; i++) {
        if (!Number.isFinite(values[i]))
            throw new RangeError(`universalInferenceMeanShiftEValue: non-finite value at test index ${i}`);
    }
    ...
    const cTrain = { i0: a0 + 1, i1: a0 + h1, pred: values[a0] };
    const tTrain = { i0: b0, i1: b0 + h2, pred: values[b0 - 1] };
    // EVAL halves (the scored data). Each predecessor is the last point of the matching train half.
    const cEval = { i0: a0 + h1, i1: a0 + n1, pred: values[a0 + h1 - 1] };
    const tEval = { i0: b0 + h2, i1: b0 + n2, pred: values[b0 + h2 - 1] };
...
//# sourceMappingURL=universal-inference-e-value.js.map