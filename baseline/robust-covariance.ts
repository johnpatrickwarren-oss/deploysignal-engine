...
export function choleskyLocal(A: ReadonlyArray<ReadonlyArray<number>>): number[][] | null {
  const n = A.length;
...
export function columnMean(rows: ReadonlyArray<ReadonlyArray<number>>): number[] {
...
export function sampleCovariance(Z: ReadonlyArray<ReadonlyArray<number>>): number[][] {
  const n = Z.length, p = Z[0].length;
  const S: number[][] = Array.from({ length: p }, () => new Array(p).fill(0));
  for (const z of Z) for (let i = 0; i < p; i++) { const zi = z[i]; for (let j = 0; j < p; j++) S[i][j] += zi * z[j]; }
  for (let i = 0; i < p; i++) for (let j = 0; j < p; j++) S[i][j] /= n;
  return S;
}

/** Mahalanobis distance² (z−mean)ᵀ Σ⁻¹ (z−mean) given Σ's Cholesky L. */
export function mahalanobisSqFromL(z: ReadonlyArray<number>, mean: ReadonlyArray<number>, L: ReadonlyArray<ReadonlyArray<number>>): number {
  const n = L.length;
...
function logDetCholesky(S: ReadonlyArray<ReadonlyArray<number>>): number | null {
  const L = choleskyLocal(S);
  if (!L) return null;
  let d = 0;
  for (let i = 0; i < L.length; i++) d += Math.log(L[i][i]);
  return 2 * d;
}
...
export function ledoitWolfShrinkage(Z: ReadonlyArray<ReadonlyArray<number>>): { cov: number[][]; lambda: number } {
  const n = Z.length, p = Z[0].length;
  ...
  muDiag /= p;
  let dSq = 0;
  for (let i = 0; i < p; i++) for (let j = 0; j < p; j++) { const fij = i === j ? muDiag : 0; const diff = S[i][j] - fij; dSq += diff * diff; }
  let bBar2 = 0;
  for (const z of Z) { let normSq = 0; for (let i = 0; i < p; i++) { const zi = z[i]; for (let j = 0; j < p; j++) { const diff = zi * z[j] - S[i][j]; normSq += diff * diff; } } bBar2 += normSq; }
  bBar2 /= (n * n);
  const bSq = Math.min(bBar2, dSq);
  const lambda = dSq > 0 ? bSq / dSq : 0;
  const cov: number[][] = Array.from({ length: p }, () => new Array(p).fill(0));
  for (let i = 0; i < p; i++) for (let j = 0; j < p; j++) { const fij = i === j ? muDiag : 0; cov[i][j] = lambda * fij + (1 - lambda) * S[i][j]; }
  return { cov, lambda };
}

// ── Croux–Haesbroeck consistency correction (from _family-c-mcd.ts) ──────────────────────────────────────────
function beasleySpringerInverseNormal(p: number): number {
  const y = p - 0.5;
  if (Math.abs(y) < 0.42) {
    const r = y * y;
...
function chiSqQuantileWH(alpha: number, k: number): number {
  const z = beasleySpringerInverseNormal(alpha);
  const a = 2 / (9 * k);
  const base = 1 - a + z * Math.sqrt(a);
  return k * base * base * base;
}
function chiSqCdfWH(q: number, k: number): number {
  if (q <= 0) return 0;
  const a = 2 / (9 * k);
...
interface CStep { mean: number[]; cov: number[][]; indices: number[]; logDet: number; }
function cStep(rows: number[][], currentMean: number[], currentCov: number[][], h: number): CStep | null {
  const L = choleskyLocal(currentCov);
  if (!L) return null;
  const distances: Array<{ idx: number; d2: number }> = [];
  for (let i = 0; i < rows.length; i++) distances.push({ idx: i, d2: mahalanobisSqFromL(rows[i], currentMean, L) });
...
function initialSubsetEstimate(rows: number[][], rng: () => number): { mean: number[]; cov: number[][] } | null {
  ...
  while (est === null && indices.size < n) {
    let added = false;
    while (!added) { const j = Math.floor(rng() * n); if (!indices.has(j)) { indices.add(j); added = true; } }
    est = expand();
  }
  return est;
}
interface FastMCDResult { mean: number[]; cov: number[][]; supportIndices: number[]; }
function fastMCD(rows: number[][], alpha: number, seed: number): FastMCDResult | null {
  const n = rows.length, p = rows[0].length;
  if (n < p + 1) return null;
  const h = Math.max(p + 1, Math.ceil(alpha * n));
  const rng = mulberry32(seed);
  const candidates: CStep[] = [];
  for (let t = 0; t < FASTMCD_N_INITIAL_SUBSETS; t++) {
    ...
    if (step2) candidates.push(step2);
  }
  if (candidates.length === 0) return null;
  candidates.sort((a, b) => a.logDet - b.logDet);
  let best: CStep | null = null;
...
function mcdReweight(rows: number[][], mcdMean: number[], mcdCov: number[][]): { mean: number[]; cov: number[][]; keptCount: number } | null {
  const p = rows[0].length;
  const cutoff = chiSqQuantile975(p);
  const L = choleskyLocal(mcdCov);
  if (!L) return null;
  const kept: number[][] = [];
  for (let i = 0; i < rows.length; i++) if (mahalanobisSqFromL(rows[i], mcdMean, L) <= cutoff) kept.push(rows[i]);
...
export function robustCovariance(rows: ReadonlyArray<ReadonlyArray<number>>, opts?: RobustCovarianceOptions): RobustCovarianceResult {
  const fn = 'robustCovariance';
  const n = rows.length;
  if (n === 0) throw new RangeError(`${fn}: rows must be non-empty`);
  const p = rows[0].length;
  if (p === 0) throw new RangeError(`${fn}: rows must have ≥ 1 dimension`);
  for (let i = 0; i < n; i++) {
    if (rows[i].length !== p) throw new RangeError(`${fn}: ragged rows — row ${i} has ${rows[i].length} dims, expected ${p}`);
    for (let j = 0; j < p; j++) if (!Number.isFinite(rows[i][j])) throw new RangeError(`${fn}: non-finite value at [${i}][${j}]`);
  }
  ...
  const lwFallback = (): RobustCovarianceResult => {
  ...
  if (n < Math.max(minPerDim * p, p + 1)) return lwFallback();
  const mcd = fastMCD(R, alpha, seed);
  if (!mcd) return lwFallback();
