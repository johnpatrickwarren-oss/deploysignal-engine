...
export function freshBaselineLifecycle(opts?: BaselineLifecycleOptions): BaselineLifecycleState {
  const window = opts?.window ?? DEFAULT_RATE_WINDOW;
...
export function updateBaselineLifecycle(state: BaselineLifecycleState, fired: boolean): BaselineLifecycleUpdate {
  const i = state.tick;
  let reRecord = false;
  if (fired) {
    ...
    let drop = 0;
    while (drop < state.alarmTicks.length && state.alarmTicks[drop] <= cutoff) drop++;
    if (drop > 0) state.alarmTicks.splice(0, drop);
