...
function genFleet(seed, mfail, step = STEP) {
    const rng = lcg(seed);
    const cm = new Array(T).fill(0);
    for (let t = 1; t < T; t++)
        cm[t] = cm[t - 1] + DRIFT * gaussian(rng);
...
function robustEValues(X) {
    return (0, common_mode_1.contaminationRobustResiduals)(X, M).map((r) => (0, nuisance_robust_bf_e_value_1.nuisanceRobustBFEValue)(r, CAL, TST));
}
function median(xs) {
...
function naiveEValues(X) {
    const t = X[0].length, n = X.length;
    const R = X.map(() => new Array(t));
    for (let j = 0; j < t; j++) {
        const med = median(X.map((row) => row[j]));
        for (let i = 0; i < n; i++)
            R[i][j] = X[i][j] - med;
    }
    return R.map((r) => (0, nuisance_robust_bf_e_value_1.nuisanceRobustBFEValue)(r, CAL, TST));
}
...
function medianDemeanEValues(X) {
    const t = X[0].length, n = X.length;
    const lvl = (0, common_mode_1.perShardLevel)(X, M);
    const R = X.map(() => new Array(t));
    for (let j = 0; j < t; j++) {
...
function fdpPower(evals, failed, q) {
    const rej = (0, e_bh_1.eBenjaminiHochberg)(evals, q).selected;
...
(0, node_test_1.test)('robustLocation: redescends — a minority of gross outliers gets weight 0 and cannot move the center', () => {
    const inliers = Array.from({ length: 40 }, () => 0);
    const withOutliers = [...inliers, 100, 120, 150]; // 3/43 gross outliers
    const loc = (0, common_mode_1.robustLocation)(withOutliers);
    strict_1.default.ok(Math.abs(loc) < 1e-6, `center must stay at the inlier mode ~0; got ${loc}`);
    ...
    strict_1.default.ok(Math.abs((0, common_mode_1.robustLocation)(moreExtreme)) < 1e-6, 'an extreme outlier must not move a redescending center');
    strict_1.default.ok(Math.abs(mean(moreExtreme)) > 1e7, 'sanity: the mean IS dragged by the same outliers');
});
(0, node_test_1.test)('robustLocation: guards and edge cases', () => {
    strict_1.default.equal((0, common_mode_1.robustLocation)([]), 0, 'empty → 0');
    strict_1.default.throws(() => (0, common_mode_1.robustLocation)([1, 2, 3], 0), RangeError, 'c must be > 0');
    // On clean Gaussian-ish data it agrees with the mean to within sampling error.
    const rng = lcg(99);
    const clean = Array.from({ length: 500 }, () => 10 + gaussian(rng));
    strict_1.default.ok(Math.abs((0, common_mode_1.robustLocation)(clean) - 10) < 0.3, 'on clean data ≈ the true center');
    strict_1.default.equal(common_mode_1.TUKEY_C, 4.685);
});
// ── 2. Demean rank-flip. ────────────────────────────────────────────────────────────────────────
(0, node_test_1.test)('contaminationRobustResiduals: demean flips a faulty-but-low-level shard from mid-pack to outlier', () => {
    ...
    for (let i = 0; i < n; i++) {
        const row = new Array(t);
        for (let k = 0; k < t; k++)
            row[k] = levels[i] + 0.5 * gaussian(rng) + (i === faulty && k >= faultTick ? step : 0);
        X[i] = row;
    }
    ...
    strict_1.default.ok(rawRank > n / 2, `by raw value the faulty shard is mid/low-pack (rank ${rawRank}/${n}), not an outlier`);
    // Level-adjusted residual at the fault tick: the faulty shard is now the clear cross-sectional outlier.
    const lvl = (0, common_mode_1.perShardLevel)(X, cal);
    strict_1.default.ok(Math.abs(lvl[faulty] - 980) < 1, 'per-shard level recovers the faulty shard’s low baseline');
    const R = (0, common_mode_1.contaminationRobustResiduals)(X, cal);
    const absResid = R.map((r) => Math.abs(r[faultTick]));
    const maxIdx = absResid.indexOf(Math.max(...absResid));
    strict_1.default.equal(maxIdx, faulty, 'after demean the faulty shard has the largest |residual| (it became the outlier)');
    strict_1.default.ok(absResid[faulty] > 5, `the fault (step ${step}) survives into the residual; got ${absResid[faulty].toFixed(2)}`);
});
(0, node_test_1.test)('contaminationRobustResiduals: guards', () => {
    strict_1.default.throws(() => (0, common_mode_1.contaminationRobustResiduals)([], 5), RangeError, 'empty');
    strict_1.default.throws(() => (0, common_mode_1.contaminationRobustResiduals)([[1, 2], [3]], 1), RangeError, 'ragged');
    strict_1.default.throws(() => (0, common_mode_1.contaminationRobustResiduals)([[1, 2, 3]], 0), RangeError, 'calLen < 1');
    strict_1.default.throws(() => (0, common_mode_1.contaminationRobustResiduals)([[1, 2, 3]], 4), RangeError, 'calLen > ticks');
    strict_1.default.throws(() => (0, common_mode_1.contaminationRobustResiduals)([[1, 2, NaN], [4, 5, 6]], 1), RangeError, 'non-finite value');
});
...
(0, node_test_1.test)('pipeline: the assembled demean+robust+BF+e-BH pipeline controls FDP ≤ q at power; the naive no-demean center does not', () => {
    ...
    strict_1.default.ok(robustFDP <= q + 0.02, `robust (demean+Tukey) FDP ${robustFDP.toFixed(3)} must be ≤ q (=${q})`);
    strict_1.default.ok(robustPow >= 0.95, `robust power ${robustPow.toFixed(3)} must stay high`);
    // The demean is the load-bearing lever: with it, even a plain median center controls FDP…
    strict_1.default.ok(medDemeanFDP <= q + 0.02, `demean+median FDP ${medDemeanFDP.toFixed(3)} must also be ≤ q (the demean is the lever)`);
    // …without it, the same e-value over a contaminated center blows the FDR (the ADR 0012 gap).
    strict_1.default.ok(naiveFDP > Math.max(robustFDP, medDemeanFDP) + 0.1, `naive no-demean FDP ${naiveFDP.toFixed(3)} must be materially worse — the demean is what closes the ADR 0012 gap`);
});
...
(0, node_test_1.test)('breakpoint: FDR controlled at a 10% fault fraction, lost at 40% (past the breakdown)', () => {
    ...
    strict_1.default.ok(lowFrac <= q + 0.02, `at 10% faults FDP ${lowFrac.toFixed(3)} must be ≤ q`);
    strict_1.default.ok(highFrac > lowFrac, `at 40% faults FDP ${highFrac.toFixed(3)} must degrade vs 10% (${lowFrac.toFixed(3)}) — the breakdown`);
});
//# sourceMappingURL=adr-0004-pr-b-contamination-robust-fleet.test.js.map