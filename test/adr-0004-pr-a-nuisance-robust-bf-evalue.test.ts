...
import {
  nuisanceRobustBFEValue,
  NUISANCE_ROBUST_BF_ENVELOPE,
  DEFAULT_TAU_MULT,
  MIN_CALIBRATION_FOR_VALIDITY,
} from '../detectors/nuisance-robust-bf-e-value';
...
function ar1Stream(
  seed: number, len: number,
  shiftAt: number, shift: number,
  varAt = -1, stdMult = 1,
): number[] {
  const rng = lcg(seed);
  const v: number[] = [];
...
function rateAtLeast(xs: number[], k: number): number { return xs.filter((x) => x >= k).length / xs.length; }

/** Collect null and mean-shift BF e-values over K seeded streams for a (m, n) regime. */
function sweep(m: number, n: number, K: number): { nulls: number[]; shifts: number[] } {
  const nulls: number[] = [], shifts: number[] = [];
  for (let s = 0; s < K; s++) {
    const vN = ar1Stream(11 + s * 7919, m + n, -1, 0);
    nulls.push(nuisanceRobustBFEValue(vN, { start: 0, len: m }, { start: m, len: n }));
    const vS = ar1Stream(9001 + s * 7919, m + n, m + 20, 4);
    shifts.push(nuisanceRobustBFEValue(vS, { start: 0, len: m }, { start: m, len: n }));
  }
  return { nulls, shifts };
}
...
for (const [label, m, n, K] of [
  ...
  test(`validity (CORRECTED 2026-07-02): bounded E[BF|H0] + Ville tail bounds — ${label}`, () => {
    ...
    for (const k of [10, 100, 1000]) {
      ...
      assert.ok(r <= 1 / k + 0.005, `P(BF≥${k}) = ${r.toFixed(4)} must be ≤ 1/${k} (+MC slack)`);
    }
...
test('power: detects a mean shift (fire rate → 1.0 at threshold 1/α)', () => {
  ...
  assert.ok(rateAtLeast(shifts, 1 / ALPHA) >= 0.99, `shift detection rate ${rateAtLeast(shifts, 1 / ALPHA)} must be ≥ 0.99`);
});

// ── 3. Same-variance scope — a variance change inflates P(fire) (documents the envelope). ──────────
test('scope: a test-window variance change inflates P(fire) above α (envelope = variance:stable)', () => {
  const m = 1500, n = 300, K = 400;
  const fireRateFor = (stdMult: number): number => {
    const es: number[] = [];
    for (let s = 0; s < K; s++) {
      const v = ar1Stream(31 + s * 7919, m + n, -1, 0, /* varAt */ m, stdMult);
      es.push(nuisanceRobustBFEValue(v, { start: 0, len: m }, { start: m, len: n }));
    }
    return rateAtLeast(es, 1 / ALPHA);
  };
  // No variance change → controlled at α; a 3×-std inflation → demonstrably elevated (the scope limit).
  assert.ok(fireRateFor(1) <= ALPHA + 0.01, 'stable variance must stay ≈ α');
  assert.ok(fireRateFor(3) > fireRateFor(1), 'a 3× variance inflation must raise the fire rate (out-of-envelope)');
});

// ── 4. Window generalization + ar1Phi override + windowing-math parity. ────────────────────────────
test('windows: arbitrary non-adjacent (cal, test) windows are handled and finite', () => {
  const v = ar1Stream(2026, 2000, -1, 0);
  // Non-adjacent: a gap between calibration and test windows.
  const e = nuisanceRobustBFEValue(v, { start: 100, len: 800 }, { start: 1500, len: 300 });
  assert.ok(Number.isFinite(e) && e >= 0, `e-value must be finite and ≥ 0; got ${e}`);
});

test('windows: ar1Phi override is honored (matches estimate when equal; differs when changed)', () => {
  ...
  const phiHat = computePerSignalAr1Phi(calValues, mean(calValues));
  const eDefault = nuisanceRobustBFEValue(v, cal, t);
  const eSamePhi = nuisanceRobustBFEValue(v, cal, t, { ar1Phi: phiHat });
  const eOtherPhi = nuisanceRobustBFEValue(v, cal, t, { ar1Phi: phiHat + 0.3 });
  assert.ok(Math.abs(eDefault - eSamePhi) < 1e-9, 'override with the estimated φ must match the default');
  assert.ok(Math.abs(eDefault - eOtherPhi) > 1e-9, 'a different φ must change the result');
});

test('windows: parity with a reference that re-derives the windowing indices (pins the index loops)', () => {
  ...
  const logMarginal = (S: number, nn: number, s2: number, tau2: number): number =>
    (tau2 * S * S) / (2 * s2 * (s2 + nn * tau2)) - 0.5 * Math.log(1 + (nn * tau2) / s2);
  const ref = (v: number[], cS: number, cL: number, tS: number, tL: number): number => {
    const calV = v.slice(cS, cS + cL);
    const phi = computePerSignalAr1Phi(calV, mean(calV));
    ...
    const tau2 = DEFAULT_TAU_MULT * s2;
  ...
  for (const seed of [1, 42, 777]) {
    for (const [cS, cL, tS, tL] of [[0, 600, 600, 200], [200, 500, 800, 300]] as const) {
      const v = ar1Stream(seed, 1300, -1, 0);
      const got = nuisanceRobustBFEValue(v, { start: cS, len: cL }, { start: tS, len: tL });
      const want = ref(v, cS, cL, tS, tL);
      assert.ok(Math.abs(got - want) < 1e-9 * Math.max(1, want), `parity seed=${seed} (${cS},${cL},${tS},${tL}): got ${got}, want ${want}`);
    }
  }
});

test('formula: the closed-form BF equals a brute-force numerical integration of the marginals (independent)', () => {
  ...
  for (const data of [v0, v]) {
    ...
    const phi = computePerSignalAr1Phi(calV, mean(calV));
    const wc: number[] = []; for (let t = cS + 1; t < cS + cL; t++) wc.push(data[t] - phi * data[t - 1]);
    const wt: number[] = []; for (let t = tS; t < tS + tL; t++) wt.push(data[t] - phi * data[t - 1]);
    const mc = mean(wc);
    const s2 = Math.max(wc.reduce((a, b) => a + (b - mc) ** 2, 0) / (wc.length - 1), 1e-9);
    const tau2 = DEFAULT_TAU_MULT * s2;
    ...
    const numLogMarg = (xs: number[]): number => {
      ...
      const logIntegrand = (mu: number): number => (mu * Sx) / s2 - (n * mu * mu) / (2 * s2) - (mu * mu) / (2 * tau2);
      // log-sum-exp trapezoid
      let maxLog = -Infinity;
      for (let i = 0; i <= steps; i++) { const l = logIntegrand(lo + i * h); if (l > maxLog) maxLog = l; }
      let acc = 0;
    ...
    const bfClosed = nuisanceRobustBFEValue(data, { start: cS, len: cL }, { start: tS, len: tL });
    assert.ok(
      Math.abs(Math.log(bfNumeric) - Math.log(bfClosed)) < 1e-3,
      `closed-form BF ${bfClosed} must match numerical ${bfNumeric} (log-diff ${Math.abs(Math.log(bfNumeric) - Math.log(bfClosed))})`,
    );
  }
});

// ── Envelope metadata is shipped (ADR 0004 — validity envelopes as first-class). ──────────────────
test('envelope: validity envelope is exported and correct', () => {
  assert.equal(NUISANCE_ROBUST_BF_ENVELOPE.baseline, 'unknown-mean-integrated');
  assert.equal(NUISANCE_ROBUST_BF_ENVELOPE.autocorrelation, 'ar1-whitened');
  assert.equal(NUISANCE_ROBUST_BF_ENVELOPE.null, 'mean-shift');
  assert.equal(NUISANCE_ROBUST_BF_ENVELOPE.variance, 'stable');
  assert.equal(NUISANCE_ROBUST_BF_ENVELOPE.minCalibration, MIN_CALIBRATION_FOR_VALIDITY);
  assert.equal(MIN_CALIBRATION_FOR_VALIDITY, 100);
});
...
test('floor: cal.len below MIN_CALIBRATION_FOR_VALIDITY throws; at/above it computes a valid e-value', () => {
  const v = ar1Stream(123, 1000, -1, 0);
  assert.throws(() => nuisanceRobustBFEValue(v, { start: 0, len: 50 }, { start: 50, len: 200 }), RangeError, 'cal=50 < floor');
  assert.throws(() => nuisanceRobustBFEValue(v, { start: 0, len: MIN_CALIBRATION_FOR_VALIDITY - 1 }, { start: MIN_CALIBRATION_FOR_VALIDITY - 1, len: 200 }), RangeError, 'floor-1');
  const e = nuisanceRobustBFEValue(v, { start: 0, len: MIN_CALIBRATION_FOR_VALIDITY }, { start: MIN_CALIBRATION_FOR_VALIDITY, len: 200 });
  assert.ok(Number.isFinite(e) && e >= 0, `at the floor a finite e-value must be produced; got ${e}`);
});
...
test('guards: invalid windows and non-finite inputs throw RangeError', () => {
  ...
  assert.throws(() => nuisanceRobustBFEValue(v, { start: 0, len: 100 }, { start: 100, len: 1 }), RangeError, 'test.len < 2');
  assert.throws(() => nuisanceRobustBFEValue(v, { start: 0, len: 100 }, { start: 0, len: 10 }), RangeError, 'test.start < 1');
  assert.throws(() => nuisanceRobustBFEValue(v, { start: 0, len: 100 }, { start: 950, len: 100 }), RangeError, 'out of bounds');
  const withNaN = v.slice(); withNaN[50] = NaN;
  assert.throws(() => nuisanceRobustBFEValue(withNaN, { start: 0, len: 200 }, { start: 200, len: 100 }), RangeError, 'NaN in cal window');
  const withInf = v.slice(); withInf[250] = Infinity;
  assert.throws(() => nuisanceRobustBFEValue(withInf, { start: 0, len: 200 }, { start: 251, len: 100 }), RangeError, 'Inf in test window');
});
