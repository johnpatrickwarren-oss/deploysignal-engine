...
function evaluateFamilyCBettingEProcess(cfg, liveMetrics, states, ctx) {
    ...
    // Project live metrics into the Family C relative-deviation vector.
    const v = liveVectorFamilyC(liveMetrics, params.mean_vector, cfg.family_c_signals ?? hotelling_1.FAMILY_C_SIGNALS);
    if (v === null)
        return null;
    ...
    if (!rffActive && (!pool || pool.length === 0)) {
        ...
        pool = (0, sequential_mmd_1.generateBaselinePool)(params, poolSize, (0, sequential_mmd_1.baselinePoolSeed)({ hour_of_day: ctx.hourOfDay, day_of_week: ctx.dayOfWeek }));
        states[poolKey] = pool;
    }
