...
function logMarginal(S, n, s2, tau2) {
...
function nuisanceRobustBFEValue(values, cal, test, opts) {
    ...
    if (cal.len < exports.MIN_CALIBRATION_FOR_VALIDITY) {
        ...
        throw new RangeError(`nuisanceRobustBFEValue: cal.len must be >= MIN_CALIBRATION_FOR_VALIDITY (`
            + `${exports.MIN_CALIBRATION_FOR_VALIDITY}) for E[BF|H0] <= 1 to hold; got ${cal.len}`);
    }
    if (test.len < 2) {
    ...
    if (cal.start < 0 || cal.start + cal.len > values.length
        ...
        throw new RangeError(`nuisanceRobustBFEValue: window out of bounds (values.length=${values.length}, `
    ...
    const tauMult = opts?.tauMult ?? exports.DEFAULT_TAU_MULT;
    if (!(tauMult > 0)) {
    ...
    if (phi === undefined) {
        ...
        phi = (0, family_a_mixture_supermartingale_1.computePerSignalAr1Phi)(calValues, calMean);
    }
...
//# sourceMappingURL=nuisance-robust-bf-e-value.js.map